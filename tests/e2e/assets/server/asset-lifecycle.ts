...
test(
  'B5: insert increments asset count',
  async ({ url }) => {
...
test(
  'B6: update tracks added/removed',
  async ({ url }) => {
...
test(
  'B7: delete decrements asset count',
  async ({ url }) => {
    const fileObj = await uploadViaApi(url)
    const AssetModel = getAssetModel()
...
test(
  'B8: shared asset reference counting',
  async ({ url }) => {
    const fileObj = await uploadViaApi(url)
    const AssetModel = getAssetModel()
...
test(
  'B9: orphan cleanup removes file and record',
  async ({ url }) => {
    const fileObj = await uploadViaApi(url)
    const AssetModel = getAssetModel()
...
test(
  'B10: dangling protection for new uploads',
  async ({ url }) => {
    const fileObj = await uploadViaApi(url)
    const AssetModel = getAssetModel()
