...
test.describe('signature forgery', () => {
  test(
    'reject patch with missing signature',
    async ({ url }) => {
  ...
  test(
    'reject patch with tampered signature',
    async ({ url }) => {
      const fileObj = await uploadViaApi(url)
      const app = AssetWidget.app!
  ...
  test(
    'reject insert with fabricated file',
    async ({ url }) => {
      const app = AssetWidget.app!
  ...
  test(
    'valid signature accepted on patch',
    async ({ url }) => {
      const fileObj = await uploadViaApi(url)
